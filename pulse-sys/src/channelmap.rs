...
pub const PA_CHANNEL_POSITION_FRONT_RIGHT: pa_channel_position_t =
    pa_channel_position_t::FrontRight;
pub const PA_CHANNEL_POSITION_FRONT_CENTER: pa_channel_position_t =
    pa_channel_position_t::FrontCenter;
pub const PA_CHANNEL_POSITION_REAR_CENTER: pa_channel_position_t =
    pa_channel_position_t::RearCenter;
...
pub const PA_CHANNEL_POSITION_FRONT_LEFT_OF_CENTER: pa_channel_position_t =
    pa_channel_position_t::FrontLeftOfCenter;
pub const PA_CHANNEL_POSITION_FRONT_RIGHT_OF_CENTER: pa_channel_position_t =
    pa_channel_position_t::FrontRightOfCenter;
...
pub const PA_CHANNEL_POSITION_TOP_FRONT_LEFT: pa_channel_position_t =
    pa_channel_position_t::TopFrontLeft;
pub const PA_CHANNEL_POSITION_TOP_FRONT_RIGHT: pa_channel_position_t =
    pa_channel_position_t::TopFrontRight;
pub const PA_CHANNEL_POSITION_TOP_FRONT_CENTER: pa_channel_position_t =
    pa_channel_position_t::TopFrontCenter;
pub const PA_CHANNEL_POSITION_TOP_REAR_LEFT: pa_channel_position_t =
    pa_channel_position_t::TopRearLeft;
pub const PA_CHANNEL_POSITION_TOP_REAR_RIGHT: pa_channel_position_t =
    pa_channel_position_t::TopRearRight;
pub const PA_CHANNEL_POSITION_TOP_REAR_CENTER: pa_channel_position_t =
    pa_channel_position_t::TopRearCenter;
...
pub unsafe fn pa_channel_map_init(m: *mut pa_channel_map) -> *mut pa_channel_map {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_init)(m)
    } else {
        std::ptr::null_mut()
    }
}

pub unsafe fn pa_channel_map_init_mono(m: *mut pa_channel_map) -> *mut pa_channel_map {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_init_mono)(m)
    } else {
        std::ptr::null_mut()
    }
}

pub unsafe fn pa_channel_map_init_stereo(m: *mut pa_channel_map) -> *mut pa_channel_map {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_init_stereo)(m)
    } else {
        std::ptr::null_mut()
    }
}
...
pub unsafe fn pa_channel_map_init_auto(
    ...
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_init_auto)(m, channels, def)
    } else {
        std::ptr::null_mut()
    }
}

pub unsafe fn pa_channel_map_init_extend(
    m: *mut pa_channel_map,
    channels: u32,
    def: pa_channel_map_def_t,
) -> *mut pa_channel_map {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_init_extend)(m, channels, def)
    } else {
        std::ptr::null_mut()
    }
}

pub unsafe fn pa_channel_position_to_string(pos: pa_channel_position_t) -> *const c_char {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_position_to_string)(pos)
    } else {
        std::ptr::null()
    }
}

pub unsafe fn pa_channel_position_from_string(s: *const c_char) -> pa_channel_position_t {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_position_from_string)(s)
    } else {
        pa_channel_position_t::Invalid
    }
}

pub unsafe fn pa_channel_position_to_pretty_string(pos: pa_channel_position_t) -> *const c_char {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_position_to_pretty_string)(pos)
    } else {
        std::ptr::null()
    }
}

pub unsafe fn pa_channel_map_snprint(
    s: *mut c_char,
    l: usize,
    map: *const pa_channel_map,
) -> *mut c_char {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_snprint)(s, l, map)
    } else {
        std::ptr::null_mut()
    }
}

pub unsafe fn pa_channel_map_parse(
    map: *mut pa_channel_map,
    s: *const c_char,
) -> *mut pa_channel_map {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_parse)(map, s)
    } else {
        std::ptr::null_mut()
    }
}

pub unsafe fn pa_channel_map_equal(a: *const pa_channel_map, b: *const pa_channel_map) -> i32 {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_equal)(a, b)
    } else {
        0
    }
}

pub unsafe fn pa_channel_map_valid(map: *const pa_channel_map) -> i32 {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_valid)(map)
    } else {
        0
    }
}

pub unsafe fn pa_channel_map_compatible(
    map: *const pa_channel_map,
    ss: *const pa_sample_spec,
) -> i32 {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_compatible)(map, ss)
    } else {
        0
    }
}

pub unsafe fn pa_channel_map_superset(a: *const pa_channel_map, b: *const pa_channel_map) -> i32 {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_superset)(a, b)
    } else {
        0
    }
}

pub unsafe fn pa_channel_map_can_balance(map: *const pa_channel_map) -> i32 {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_can_balance)(map)
    } else {
        0
    }
}

pub unsafe fn pa_channel_map_can_fade(map: *const pa_channel_map) -> i32 {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_can_fade)(map)
    } else {
        0
    }
}
...
pub unsafe fn pa_channel_map_can_lfe_balance(map: *const pa_channel_map) -> i32 {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_can_lfe_balance)(map)
    } else {
        0
    }
}

pub unsafe fn pa_channel_map_to_name(map: *const pa_channel_map) -> *const c_char {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_to_name)(map)
    } else {
        std::ptr::null()
    }
}

pub unsafe fn pa_channel_map_to_pretty_name(map: *const pa_channel_map) -> *const c_char {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_to_pretty_name)(map)
    } else {
        std::ptr::null()
    }
}

pub unsafe fn pa_channel_map_has_position(
    map: *const pa_channel_map,
    p: pa_channel_position_t,
) -> i32 {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_has_position)(map, p)
    } else {
        0
    }
}

pub unsafe fn pa_channel_map_mask(map: *const pa_channel_map) -> pa_channel_position_mask_t {
    if let Some(functions) = ffi::get_functions() {
        (functions.pa_channel_map_mask)(map)
    } else {
        0
    }
}
